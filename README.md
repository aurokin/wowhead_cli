...
Use `--hydrate-linked-entities` to also write bounded local entity payloads under `entities/<type>/<id>.json` plus `entities/manifest.json`. Hydration reuses the normalized `entity` contract and defaults to `spell,item,npc` when enabled; use `--hydrate-type` and `--hydrate-limit` to narrow it. Hydration now checks the normalized entity cache first, then falls back to live fetches only on cache misses. Hydrated entity manifests track per-entity `stored_at` timestamps and `storage_source`, while bundle manifests expose `hydration.source_counts`, so later refreshes and inspections can distinguish cache reuse, bundle-file reuse, and live fetches.
Use `guide-bundle-list` to discover exported bundles under `./wowhead_exports/` or another root. It now includes per-bundle `freshness` and `hydration` summaries so agents can see whether a bundle is stale, whether hydration is enabled, how many entities were hydrated, and whether those hydrated rows came from cache reuse, bundle-file reuse, or live fetches. Use `--max-age-hours` to change the freshness threshold used by those summaries. Bundle exports and refreshes now also maintain a root-level `index.json`, and `guide-bundle-list` prefers that index when it is present and valid.
Use `guide-bundle-refresh` to refresh an existing bundle in place. If `--max-age-hours` is omitted, the default freshness window is 24 hours, so agents can refresh safely without always specifying a threshold. When a bundle includes hydrated entities, refresh now selectively rehydrates only the entity payloads whose `stored_at` timestamp is stale unless you force a full refresh.
