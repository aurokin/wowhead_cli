...
Use `guide-bundle-refresh` to refresh an existing bundle in place. If `--max-age-hours` is omitted, the default freshness window is 24 hours, so agents can refresh safely without always specifying a threshold.
