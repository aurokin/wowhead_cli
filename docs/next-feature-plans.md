...
- `guide-export` can now write bounded hydrated linked-entity payloads under `entities/` using the normalized entity contract, it checks the normalized entity cache before live fetches, and bundle manifests can now report whether hydration rows came from cache reuse, bundle-file reuse, or live fetches
