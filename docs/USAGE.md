...
- `search` and `resolve` now also clean out low-value leading family hint terms like `faction`, `lore`, `guide`, and `zone` when there is a stronger article target underneath, and report the cleanup in `excluded_terms`
...
- `reference` metadata is now useful beyond API pages: programming howtos, API-change pages, class pages, faction pages, zone pages, expansion pages, systems pages, guide pages, and lore pages all expose at least a family-aware summary, and some pages also expose `patch_changes`, `see_also`, and `references`
